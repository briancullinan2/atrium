...
public class ModelAccessor<TModel, TReturn>(EntityMetadata<TModel> model, Func<EntityMetadata<TModel>, string?, TReturn?> selector) where TModel : Entity<TModel>, IEntity<TModel> where TReturn : struct
{
    ...
    public TReturn? this[Expression<Func<TModel, dynamic?>> property]
    {
        get
        {
            return _selector(_model, FindProperty(property)?.Name);
        }
    }
    ...
    public static PropertyInfo? FindProperty<T>(Expression<Func<TModel, T>> expression)
    {
...
public class EntityMetadata<T> : EntityMetadata where T : Entity<T>, IEntity<T>
{
